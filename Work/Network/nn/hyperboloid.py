...
data_sep=5000 #Used for seperating training and test data
training=data[data_sep:]


#data_sep=450
#training=Bdata[:data_sep]
...
w_2=np.random.randn(8,N_PAR) #Order is (output, input) i.e. (to, from)
#w_3=np.random.random((8,8))
w_4=np.random.random((1,8))
biases_2=np.random.random((8,1))
        ...
        for mB in mini_batch:
            ...
            for mBB in range(len(mB[0])):
                ...
                #for X_,Z in mB:
                    ...
                    delta_nabla_4W=np.dot(delta_4,a_2.transpose())+delta_nabla_4W*M #need to make sure right matrix is transposed!
                    delta_nabla_4B=delta_4+delta_nabla_4B*M
                    
                    #delta_nabla_3W=np.dot(delta_3,a_2.transpose())
                    #delta_nabla_3B=delta_3
                
                    delta_nabla_2W=np.dot(delta_2,x.transpose())+delta_nabla_2W*M
                    delta_nabla_2B=delta_2+delta_nabla_2B*M
                    #updat weights
                    learnrate=3/20#3/len(mB[0])
...
# plt.scatter([i[1] for i in x_set],out_set,c=y,s=20)
# plt.gray()
# #plt.plot(space,y_set,'-')
# plt.show()
